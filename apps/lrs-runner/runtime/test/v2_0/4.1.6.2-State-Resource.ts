...
const xapiRequests = xapiRequestsImport as typeof xapiRequestsImport;
...
describe("State Resource Requirements (Communication 2.3)", () => {
  ...
  describe("The LRS shall include a Last-Modified header indicating when the document was last modified.", () => {
    ...
    beforeAll(async () => {
      await xapiRequests.deleteDocument(resourcePath, resourceParams as unknown as Record<string, string>);
      await xapiRequests.postDocument(resourcePath, document, resourceParams as unknown as Record<string, string>);
    });

    it("Returns a Last-Modified header at all", async () => {
      let res = await xapiRequests.getDocuments(resourcePath, resourceParams as unknown as Record<string, string>);
    ...
    it("Updates the Last-Modified value when the corresponding document is updated.", async () => {
      let originalDocRes = await xapiRequests.getDocuments(
        resourcePath,
        resourceParams as unknown as Record<string, string>,
      );
      await xapiRequests.delay(1500);

      let updateRes = await xapiRequests.postDocument(
        resourcePath,
        updatedDocument,
        resourceParams as unknown as Record<string, string>,
      );
      ...
      let updatedDocRes = await xapiRequests.getDocuments(
        resourcePath,
        resourceParams as unknown as Record<string, string>,
      );
    ...
    //     };
    //     delete (groupParams as Record<string, unknown>)["stateId"];
